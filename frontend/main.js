(function () {
  ...
  async function loadMenuForRestaurant(restId) {
    ...
    try {
      ...
      const data = menuDataCache[restId];
          ...
          return `
            <article class="menu-card" data-id="${item.id}" data-category="${item.category}" data-price="${item.price}" data-restaurant="${item.restaurant_id || restId}">
              <div class="card-art">
                ...
                ${item.image_url
                  ? `<img src="${item.image_url}" alt="${escapeHtml(dishName)}" loading="lazy" width="400" height="260">`
  ...
  async function openDetailModal(id) {
    try {
      ...
      if (modal && body) {
        body.innerHTML = `
          ${item.image_url ? `<img src="${item.image_url}" alt="${escapeHtml(dishName)}" style="width: 100%; height: 200px; object-fit: cover; border-radius: var(--radius-sm); margin-bottom: 16px;">` : ''}
